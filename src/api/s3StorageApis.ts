import {useStorage, useLogin} from '@/store';
...
import {ENV, ADMIN_PROJECT_ID} from '@/constants/Constants';
...
const getS3Object = () => {
  ...
  return new AWS.S3({
    region: 'default',
    endpoint: ENV.ENDPOINT_CLOUD_STORAGE,
    credentials: {
...
const fetchS3Credential = async () => {
  const projectId: string = ADMIN_PROJECT_ID;
  ...
  return res.map((el) => {
    ...
    el.lastModified =
      typeof el.LastModified !== 'undefined'
        ? formatDate(String(el.LastModified))
        : '-';
    return el;
  });
};
...
export const fetchS3UsageInfo = async () => {
  ...
  const projectId: string = ADMIN_PROJECT_ID;
  const url = `/sss/cs/api/v1/project/${projectId}`;
