...
const emits = defineEmits(['create-bucket', 'close-dialog']);
...
const submitAction = () => {
  if (
    ...
    emits('close-dialog', uploadStatus.value !== UPLOAD_STATUS.INIT);
  }
};

const closeAction = () => {
  if (uploadStatus.value === UPLOAD_STATUS.ERROR) {
  ...
  if (file.isFolder) {
    ...
    abortKeys.forEach((key) => {
      if (
        ...
        s3UploadRequest.value[key].abort();
      }
    });
  } else {
    if (
      ...
      s3UploadRequest.value[fileKey].abort();
    }
  }
};
      ...
      const readEntriesFunc = async (folderIndex: number) =>
        ...
        dirReader.readEntries(async (entries: FileSystemEntry[]) => {
          if (entries.length > 0) {
            ...
            readEntriesFunc(folderIndex);
          } else {
            resolve(fileTree);
          }
        });
      readEntriesFunc(fileTree.length - 1);
    }
  });
};
  ...
  >
    <template #info>
      <v-row no-gutters class="px-4">
        <v-col cols="4" sm="2" class="content-key">
          {{ $t('basic.name.type', {type: $t('s3.bucket')}) }}
        </v-col>
        <v-col cols="8" sm="4" class="content-value">
          {{ bucketName }}
        </v-col>
    ...
    <template #default>
      ...
      <div @dragenter="onDragEnter">
        <v-row no-gutters class="border align-center header-row">
          <v-col cols="4">{{ $t('basic.name') }}</v-col>
          <v-col cols="3">
            {{ $t('basic.size') }}
          </v-col>
