import {useGlobal, useStorage} from '@/store';
...
const s3BucketList: Ref<Array<Record<string, any>>> = ref([]);
const s3ObjectList: Ref<Array<Record<string, any>>> = ref([]);
    ...
    successCallback: (res: {
      ...
      s3ObjectList.value =
        isFetchMoreAction && curContinuationToken.value
          ? s3ObjectList.value.concat(
              res.response as Array<Record<string, any>>,
            )
          : res.response;
...
const execFetchS3BucketAcl = async (
  ...
  await makeApiCallWithoutProgress({
    ...
    successCallback: (res: any) => {
      ...
      const grantee: Array<{ID: string}> = grants.map(
